...
public struct IndeterminateProgressView<Label>: View where Label: View {
    ...
    private func offset(in geometry: GeometryProxy) -> CGSize {
        ...
        let lowerBound = -indicatorWidth * 0.95
...
#Preview {
    VStack {
#if os(macOS)
        ProgressView("Default Linear Progress")
            .progressViewStyle(.linear)
#else
        ProgressView("Default Linear Progress", value: 1.0 / 3)
            .progressViewStyle(.linear)
#endif
        IndeterminateProgressView("Indeterminate Progress")
    }
    .padding()
}
